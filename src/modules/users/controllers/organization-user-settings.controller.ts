...
export class OrganizationUserSettingsController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create org-user settings record (ADMIN/SUPER_ADMIN only)' })
  ...
  @ApiOperation({ summary: 'Get settings for a user in a specific org (ADMIN/SUPER_ADMIN only)' })
  findByUserAndOrg(
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get settings by record ID (ADMIN/SUPER_ADMIN only)' })
  findOne(@Param('id') id: string) {
    return this.orgUserSettingsService.findById(id);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Update org-user settings (ADMIN/SUPER_ADMIN only)' })
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Delete org-user settings (ADMIN/SUPER_ADMIN only)' })

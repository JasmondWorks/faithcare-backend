...
export class SalvationRecordController {
  ...
  @Post()
  @ApiOperation({ summary: 'Record a salvation decision (ADMIN/SUPER_ADMIN only)' })
  create(
  ...
  @Get()
  @ApiOperation({ summary: 'List all salvation records for the organization (ADMIN/SUPER_ADMIN only)' })
  findByOrganization(@Param('organizationId') organizationId: string) {
    return this.salvationRecordService.findByOrganization(organizationId);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get a salvation record by ID (ADMIN/SUPER_ADMIN only)' })
  findOne(@Param('id') id: string) {
    return this.salvationRecordService.findById(id);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Update a salvation record (ADMIN/SUPER_ADMIN only)' })
  update(@Param('id') id: string, @Body() updateDto: UpdateSalvationRecordDto) {
    return this.salvationRecordService.update(id, updateDto);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Delete a salvation record (ADMIN/SUPER_ADMIN only)' })

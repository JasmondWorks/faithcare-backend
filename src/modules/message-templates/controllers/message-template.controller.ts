...
export class MessageTemplateController {
  ...
  @ApiOperation({
    ...
    description:
      'Read-only reference templates provided by the platform. Use these as a starting point when creating org-specific templates.',
  })
  ...
  @ApiOperation({
    summary:
      "List the authenticated org's custom templates plus all presets (ADMIN only)",
    description:
      "Returns both the org's own custom templates and the global system presets in a single list.",
  })
  @ApiResponse({
    status: 200,
    description: 'Combined list of org templates and presets',
  })
  ...
  @ApiOperation({
    ...
    description:
      "Creates a fully editable template scoped to the authenticated admin's organization.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Updates the template. Only templates belonging to the authenticated admin's organization can be edited. System presets cannot be modified.",
  })
  ...
  update(
    ...
    return this.templateService.updateOrgTemplate(
      user.organizationId!,
      id,
      dto,
    );
  ...
  @ApiOperation({
    ...
    description:
      "Soft-deletes the template. Only templates belonging to the authenticated admin's organization can be deleted. System presets cannot be deleted.",
  })
